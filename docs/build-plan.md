...
6. **BuddyBoss deltas** - BUILT (verification pending a BuddyBoss fixture):
   ...
   - *Forums -> Jetonomy* - NOT built yet. Needs Jetonomy active on the target plus a bbPress fixture; deferred to avoid guessing against an engine that is not present.
   - Verification: needs a BuddyBoss site with real `bp_media`/`bp_video` rows and attachment files. To be run on reign-release once BuddyBoss demo data + BuddyNext are set up there.
