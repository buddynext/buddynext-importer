...
5. **Friends -> connections** - DONE. `buddynext_service( 'connections' )`: confirmed friendship -> `send_request` + `accept_request` (accepted), pending -> `send_request` only; idempotent via `are_connected` + id-map. CLI `migrate-friends` + REST `/step` phase `friends`. Verified end-to-end: 91 friendships -> 91 connections, all 91 pairs confirmed connected via `are_connected()`, then cleaned up.
6. **BuddyBoss deltas** - BUILT (verification pending a BuddyBoss fixture):
   - *Field types* - `gender` + `member-types` already flow through the inherited profile import (gender -> select with options; member-types is skipped); `social-networks` (an associative network -> URL map) lands in a `textarea` as readable "Network: URL" lines.
   - *Activity media* - `BuddyBossAdapter::activity_media()` reads `bp_media_ids` / `bp_video_ids` activity meta -> `bp_media` / `bp_video` -> WP `attachment_id`; `ActivityWriter` ingests each attachment (a copy, leaving the original intact) through the `MediaClient::upload()` seam (WPMediaVerse) into media ids and passes them to `PostService::create( ['media_ids' => ...] )`. Wired into the activity import so it no-ops on plain BuddyPress.
   - *Forums -> Jetonomy* - DONE (Jetonomy-gated). bbPress forum (`wp_posts`, visibility in `post_status`) -> Jetonomy space under an "Imported Forums" category; topic -> discussion post; reply -> reply (threaded via `_bbp_reply_to`). All through Jetonomy's Journey API (`Taxonomy_Journey`/`Space_Journey`/`Content_Journey`). CLI `migrate-forums`, folded into `migrate-all` when Jetonomy is active; REST `/step` phase `forums` (stages forums/topics/replies).
   - Verification: DONE on reign-release with real BuddyBoss demo data. Activity media: 3 photos -> 3 `wp_mvs_media_index` records -> post 211 (`media_ids=[1,2,3]`). Forums: 11 forums + 43 topics + 172 replies into Jetonomy. Two display bugs found in live inspection and fixed: source data is slashed (`wp_unslash` on all source text reads) and BuddyPress content is HTML while BuddyNext escapes content (HTML -> text conversion in ActivityWriter).
